                    ...
                    {history.length > 0 && (
                        history.map((item, index) => (
                            <div className="history__table-row" key={"history" + index}>
                                <div className="history__table-data"> <img src={item.img} alt="" /></div>
                                <div className="history__table-data"> {item.idcoin}</div>
                                <div className="history__table-data"> {formatToCurrency(item.cantidad)}</div>
                                <div className="history__table-data"> USD {formatToCurrency(item.precioCompra)}</div>
                                <div className="history__table-data"> {item.tipo}</div>
                                <div className="history__table-data"> {convertDate(item.fecha)}</div>
                            </div>
                        ))
                    )}
                </div>
